...
from document_processor import DocumentProcessor, extract_text, check_dependencies
...
try:
    # Load Doc2Vec model
    doc2vec_model_path = r"D:\logs\cv_match_API\model\cv_job_maching.model"
    ...
    # Load Logistic Regression model (nếu có)
    try:
        lr_model = joblib.load(r"D:\logs\cv_match_API\model\cv_job_maching.model")
    ...
    try:
        ...
        if data.method in ["doc2vec", "both"] and doc2vec_model is not None:
            try:
                ...
                doc2vec_similarity = 100 * (np.dot(np.array(v1), np.array(v2))) / (norm(np.array(v1)) * norm(np.array(v2)))
