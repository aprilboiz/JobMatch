...
function EditJobContent({ params }: EditJobPageProps) {
    ...
    const loadFormOptions = async () => {
        try {
            ...
            if (jobTypesRes.success && jobCategoriesRes.success) {
                ...
                await loadJob() // Load job data after options are loaded
            }
        } catch (error) {
            toast({
                variant: "destructive",
    ...
    const loadJob = async () => {
        try {
            const { data: job } = await apiClient.getJob(params.id)
            setFormData({
                title: job.title,
                jobType: job.jobType as JobFormData["jobType"],
                jobCategory: job.jobCategory,
                salary: {
                    salaryType: job.salary.salaryType,
                    minSalary: job.salary.minSalary,
                    maxSalary: job.salary.maxSalary,
                    currency: job.salary.currency || "USD",
                    salaryPeriod: job.salary.salaryPeriod || "ANNUAL"
                },
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await apiClient.updateJob(params.id, formData)
            toast({
                title: "Success",
        ...
        <div className="min-h-screen bg-gradient-to-br from-background via-muted/20 to-muted/40">
            <div className="container py-8">
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <p className="text-muted-foreground mb-8">Update the job posting details</p>
